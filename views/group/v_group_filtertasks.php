<?php $db_user_table = $_SESSION['group_table']; ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
            ...
            <div class="members-body">
                            ...
                            if ($stmt = $conn->prepare("SELECT id, title FROM $db_user_table WHERE status=? ORDER BY id LIMIT ?,?"))
                            {
                                ...
                                if ($stmt->num_rows > 0)
                                {
                                    while($stmt->fetch()) 
                                    {
                                        ...
                                        echo "<tr><td><a href='group_selected_task.php?id=$id'>BUG-$id: $title</a><span class='display-$class'>$status</span></td></tr>";
                                    }
                                    $stmt->free_result();
                                    $stmt->close();
                                    $conn->close();
                ...
                <?php
                    ...
                    */
                    ...
                    for($i=1;$i<=$count;$i++)
                    {
                        echo "<a style='margin-right:5px;text-decoration:underline;' href='group_users.php?page=$i&filtertasks=$status'>$i</a>";
                    }
                ?>
            </div>
        </div>
